        ...
        {SERVICES.map((s, i) => {
          ...
          return (
            ...
            >
              <div className={i % 2 === 1 ? "lg:order-2" : ""}>
                <div className="relative group">
                  ...
                  <div className="aspect-[4/3] glass-panel p-2 rounded-[3rem] overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-tr from-primary/10 to-transparent mix-blend-overlay z-10 rounded-[2.5rem]" />
                    <img
                      src={s.image}
                      alt={s.title}
                      loading="lazy"
              ...
              <div>
                <div className="inline-flex size-14 rounded-2xl bg-primary/10 items-center justify-center mb-6 border border-primary/20 shadow-[0_0_15px_rgba(16,185,129,0.15)] text-primary">
                  <Icon className="size-6" />
                </div>
                <h2 className="font-display text-3xl lg:text-5xl text-balance text-white">{s.title}</h2>
                <p className="mt-6 text-lg text-foreground/70 leading-relaxed">{s.long}</p>
                <ul className="mt-8 space-y-4">
                  {s.bullets.map((b) => (
                    <li key={b} className="flex gap-4 items-center text-white/90">
